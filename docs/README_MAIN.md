...
# 强制覆盖
fzj decrypt -i file.fzj -o out.txt -p priv.pem -s pub.pem --force
# 详细输出
...
fzj encrypt -i backup.tar.gz -o backup.fzj -p backup_public.pem -s backup_private.pem
...
1. **密钥管理**
   ...
   - 使用密钥缓存功能，避免频繁读取磁盘
   - 定期清理缓存：`fzj keymanage -a clear-cache`
...
- ✅ **错误处理增强**: 改进文件关闭和输入验证
- ✅ **依赖更新**: circl v1.6.2, pflag v1.0.10, x/sys v0.39.0
- ✅ **代码健壮性**: 提升异常处理和稳定性
