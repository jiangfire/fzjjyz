...
- **输入验证增强** (`cmd/fzj/utils/errors.go`)
...
- **签名验证强化** (`internal/zjcrypto/operations_shared.go`)
  - 修复“签名剥离绕过”风险
  - 当提供验签公钥时，密文必须包含有效签名
  - 增加签名长度与固定签名大小校验

- **默认输出路径安全收敛** (`cmd/fzjjyz/decrypt.go`)
  - `decrypt` 未指定 `--output` 时，仅使用头部文件名的 `basename`
  - 阻断通过头部文件名触发的路径穿越写入风险

- **明文权限收紧** (`internal/zjcrypto/operations_shared.go`)
  - 解密输出文件权限从 `0644` 调整为 `0600`
  - 降低本机多用户场景下的明文可见范围

- **临时明文文件风险降低** (`cmd/fzjjyz/decrypt_dir.go`)
  - `decrypt-dir` 临时 ZIP 改为随机文件名
  - 读取后尽早删除临时明文文件，缩短驻留窗口

### Fixed

- **功能与文档一致性修复** (`cmd/fzjjyz/keymanage.go`)
  - 实现 `keymanage -a cache-info`
  - 同步中英文帮助文案与错误提示

- **回归测试补充**
  - 新增签名剥离攻击回归测试
  - 新增默认输出路径清洗测试
  - 新增 CLI `cache-info` 集成测试
...
#### 核心功能扩展
- **目录加密命令** (`cmd/fzj/encrypt_dir.go`)
  - `encrypt-dir` 命令支持整个目录打包加密
  - 自动递归扫描子目录
  - ZIP 归档后再加密
  - 保持完整目录结构

- **目录解密命令** (`cmd/fzj/decrypt_dir.go`)
...
1. **非 constant 格式字符串** (9 个错误)
   ...
   - `cmd/fzj/decrypt.go`: 1 个
   - `cmd/fzj/decrypt_dir.go`: 1 个
   - `cmd/fzj/encrypt.go`: 1 个
   - `cmd/fzj/encrypt_dir.go`: 1 个
   - `cmd/fzj/keygen.go`: 1 个
   - `cmd/fzj/keymanage.go`: 2 个
...
- `cmd/fzj/encrypt_dir.go` - 目录加密
- `cmd/fzj/decrypt_dir.go` - 目录解密
...
```bash
# 目录加密
fzj encrypt-dir -i ./project -o backup.fzj -p pub.pem -s priv.pem

# 目录解密
fzj decrypt-dir -i backup.fzj -o restored -p priv.pem -s pub.pem
...
#### CLI 增强
- **错误信息改进** (`cmd/fzj/encrypt.go`, `decrypt.go`)
  - 详细错误诊断
  - 解决方案提示
  - 安全建议
