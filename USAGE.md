...
本文档提供 fzjjyz 的完整使用指南，包括所有命令的详细说明、参数解释和实际示例。
...
### 语法
```bash
fzjjyz encrypt -i <输入文件> -o <输出文件> -p <公钥> -s <签名私钥> [flags]
```
...
### 语法
```bash
fzjjyz decrypt -i <输入文件> -o <输出文件> -p <私钥> -s <验证公钥> [flags]
```
...
**语法**:
```bash
fzjjyz keymanage -a export -s <私钥文件> -o <输出公钥文件>
```
...
**语法**:
```bash
fzjjyz keymanage -a verify -p <公钥文件> -s <私钥文件>
```
